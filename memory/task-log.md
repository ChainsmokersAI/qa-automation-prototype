...
- **접근**: 기존 4개 스킬 패턴 + 시뮬레이션 출력 포맷 분석 → 설계안 제시 (판정 등급, 저장 위치, 추가 기준, 큐레이션) → PM 피드백 4건 반영 → Plan 모드 → 승인 → 적용
- **결과**:
  - **`.claude/skills/evaluate-results/SKILL.md`** 신규 생성 (Step 0~5, 6단계 워크플로우)
    - Step 0: 사전 확인 (시뮬레이션 결과 존재, 재평가 여부 감지)
    - Step 1: 평가 설정 — 1a) 대상 선택 1b) 공통 평가 기준 1c) 엄격도/특별 초점
    - Step 2: LLM-as-judge 실행 — 턴별 응답 분석(보조) + TC/공통 기준 판정(주) + TC 레벨 3단계 판정
    - Step 3: 결과 저장 — tc-*.md `## 평가 대기` → `## 평가 결과` 인플레이스 교체 + summary.md 종합 큐레이션
    - Step 4: PM 보고 + 재평가 대응
    - Step 5: 메모리 업데이트
  - **CLAUDE.md** 23행: `*(예정)*` → `/evaluate-results` (파이프라인 테이블 5행 갱신)
  - **memory/MEMORY.md**: 상태·구현 목록·최근 변경·다음 할 일 갱신
- **주요 결정**:
  - 판정 등급 3단계 (Pass / Partial / Fail) — 멀티턴 TC에서 일부 기준만 실패하는 경우 구분 필요
  - 인플레이스 교체 — 시뮬레이션 원본 데이터(대화 기록)는 보존, `## 평가 대기` 섹션만 교체. 재평가 시 덮어쓰기 (git 이력으로 복원 가능)
  - 공통 평가 기준 — PM이 TC 기준 외 추가 기준 지정 가능, 모든 TC에 일괄 적용. 모호한 기준은 에이전트가 구체화 후 PM 확인
  - 엄격도 3단계 (관대/보통/엄격) — 테스트 목적(프로토타입 vs 출시 전)에 따라 판정 수위 조절
  - 종합 큐레이션 — summary.md에 강점/약점/공통 패턴/PM 권고 사항 서술형 섹션 포함
  - TC flow 이탈 — 자동 감점하지 않되, 테스트 목적 검증 불가 시 "판정 불가 (flow 이탈)" 표기
  - `[x]`/`[ ]`/`[~]` 표기법 — 기존 TC 체크리스트 포맷 재활용 + 판정 불가 `[~]` 확장
- **검증**: 스킬 목록에 `/evaluate-results` 자동 등록 확인, CLAUDE.md 파이프라인 테이블 갱신 확인
- **교훈**: 별도 기록 없음 — PM과 사전 설계 논의가 충분해 구현 중 추가 이슈 없었음
